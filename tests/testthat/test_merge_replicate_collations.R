...
test_that("parallel merge passes Welford merge count, not replicate index", {
  recorded <- list()
  results <- list(
    collate = function(r, tm, n, calc_impacts) {
  ...
  out_r2 <- list(
    r = 2L,
    collations = list(
      list(r = 2L, tm = 0L, n = 1L, calc_impacts = NULL),
      list(r = 2L, tm = 1L, n = 1L, calc_impacts = NULL)
    )
  )
  out_r1 <- list(
    r = 1L,
    collations = list(
      list(r = 1L, tm = 0L, n = 2L, calc_impacts = NULL)
    )
  )
